...
# =========================
# Login gate (same style as your other app)
# =========================
def _require_login(logo_base64: str) -> None:
    ...
    if not st.session_state.auth:
        # Header logo on login page (same size as main header)
        if logo_base64:
            st.markdown(
                f"""
        ...
        if st.button("Login", type="primary"):
            allowed = st.secrets.get("users", {})  # expected: { "username": "password", ... }
            if isinstance(allowed, dict) and u in allowed and p == allowed[u]:
