#!/usr/bin/env python3
# CLINIQ • SOP Navigator + Benchmark Module (showcase-ready, fixed single-user login)
...
def require_login():
    ...
    if st.button("Log in", type="primary", key="login_btn"):
        ...
        if u.strip() == allowed_user and p.strip() == allowed_pass:
...
# ------------------ CAR-T BENCHMARK SCENARIOS (primary dataset) ------------------
BENCHMARK_SCENARIOS = [
    # Family A (20)
...
def main():
    ...
    with tab_ask:
        ...
        if st.button("Submit", key="submit_btn", type="primary") and question.strip():
            ...
            # Simple FAQ match (current behavior)
            if not sel_df.empty:
                ...
                if best and score >= 0.75:
                    ...
                    st.session_state["chat"].append(
                        {"role": "assistant", "content": "I couldn’t find a close FAQ match. Try changing the wording or category."}
                    )
            else:
    ...
    with tab_bench:
        ...
        st.write(
            "Use this to collect your CRC + QA scoring on the 50 de-identified CAR-T scenarios "
            "and export results."
        )
